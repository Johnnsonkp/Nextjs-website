...
function LandingPage() {
  return (
        <div className="bg-[#F5F5F5] dark:bg-[#1C1C1C] relative flex items-center justify-center overflow-hidden z-0 w-[95%] m-[auto]">
            <div className="relative mx-auto h-full px-4  pb-20   md:pb-10 sm:max-w-xl md:max-w-full md:px-24 lg:max-w-screen-xl lg:px-8">
                <div className="flex flex-row items-center justify-around lg:flex-row py-20 ml-7">
                    <div className=" relative flex-[0.5]">
                        <div className=" absolute top-0 -left-48 z-0  opacity-50 ">
                            ...
                            <div className={`${classes.dotPattern} w-36 z-0 h-full object-fill fill-y text-y`}></div>
                        </div>
                        <div className="lg:max-w-xl lg:pr-5 relative z-40">
                            <p className="flex text-sm uppercase text-g1 ">Chinonso John Nkpolukwu</p>
                            <h2 className="mb-6 max-w-lg text-5xl font-light leading-snug tracking-tight text-g1 sm:text-7xl sm:leading-snug">
                                Frontend Developer
                                <span className="my-1 inline-block border-b-8 border-g4 bg-white dark:bg-[#1C1C1C] px-4 font-bold text-g4 animate__animated animate__flash">Shopify Developer</span>
                            </h2>
                            <p className="text-base text-gray-700 dark:text-[#f4f4f4]">A dynamic Melbourne based Frontend Developer well versed in React, Ruby on Rails and Shopify Web Development. 
                            My approach involves leveraging technology into achieving the best results.</p>
                            <div className="mt-10 flex flex-row items-center md:flex-row align-middle">
                                
                                <Link href="/projects" className=" mr-3 inline-flex h-12 w-full flex-[0.5] items-center justify-center rounded bg-green-600 px-6 font-medium tracking-wide text-white shadow-md transition hover:bg-blue-800 focus:outline-none md:mr-4 md:mb-0 md:w-auto">
                                    View My Work
                                </Link>
                                {/* <Link href="/about" aria-label="" className="group inline-flex items-center font-semibold text-g1">Resume <LinkArrow className={"w-6 ml-1"} />
                                ...
                                {/* <Link href="/software-dev-portfolio-2024-2.pdf" aria-label="" className="group inline-flex items-center font-semibold text-g1 text-light p-2.5 px-6 rounded-lg text-lg font-semibold hover:bg-light hover:text-dark border border-solid border-transparent hover:border-dark dark:bg-light dark:text-dark hover:dark:bg-dark hover:dark:text-light hover:dark:border-light bg-dark">Resume 
                                ...
                                <Link
                                    ...
                                    target={"_blank"}
                                    className="flex items-center bg-dark text-light p-2.5 px-6 rounded-md text-lg font-semibold hover:bg-light hover:text-dark border border-solid border-transparent hover:border-dark dark:bg-light dark:text-dark hover:dark:bg-dark hover:dark:text-light hover:dark:border-light"
                                    download={true}
                                >
                    ...
                    <div className="relative lg:ml-32 lg:block lg:w-1/2 flex-[0.4]">
                        <div className="abg-orange-400 mx-auto w-fit  rounded-[6rem] rounded-br-none rounded-tl-none border-4 border-[#EBF0F5]-500 relative">
                            <div className='border-4 border-[#E9E9E9]-500 rounded-[6rem] overflow-hidden absolute bottom-[-2%] left-[-18%] bg-[#E9E9E9]'>
                                <Image src={smallProfilePic} className='w-[160px] h-[160px] p-0 object-fit'/>
                            </div>
                            <Image priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw" className='w-[400px] h-[400px] object-cover' src={profilePhoto} />
                        </div>
                    </div>
                </div>
            </div> 
            <Link 
                href="/about" 
                className="hidden text-9xl varient absolute top-6 left-1/4 text-g/10 z-40"
            >
                About Us
            </Link>
